...
def build_static_grid(items: list[dict]) -> str:
    ...
    for it in items[:STATIC_CELLS]:
        ...
        cells.append(
            f'<a class="cell" role="listitem" href="{asset_url(it["file"])}">'
    ...
    if rest:
        links = "".join(
            f'<li><a href="{asset_url(it["file"])}">{html.escape(describe(it))}</a></li>' for it in rest
        )
        cells.append(
...
# Content-hashed filenames — safe to cache hard and forever.
/wallpapers/*
